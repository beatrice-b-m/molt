...
use crate::kernel::{KernelManager, KernelResponse, KernelState};
...
pub async fn ensure_kernel(
    ...
    kernel_manager.ensure_kernel(tab_index).await
}
...
pub async fn restart_kernel(
    ...
    kernel_manager.restart(tab_index).await
}
...
pub async fn get_kernel_status(
    ...
    Ok(kernel_manager.get_state(tab_index).await)
}
...
pub async fn get_config_warning(
    ...
    let warning = config_state.warning.lock().map_err(|e| e.to_string())?;
    Ok(warning.clone())
}
